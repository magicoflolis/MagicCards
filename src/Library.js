    ...
    const database = _default.database.map((data) => {
      ...
      if (db.type === 'Characters') {
        db.instruction.ai = p(
          ...
          // '- Genitalia: Virgin per part; non-virgin: partner if known',
        ...
        // Do NOT invent traits. Prefer known and non-duplicates. If in doubt, omit.
        // db.instruction.ai = p(
        //   'Update PList from "$1" without repetition. Complete partials. Be concise/grounded. Include current context and memory.',
        //   '',
        //   'PList Rules:',
        ...
        //   '- Merge similar traits (Leven<3)',
        //   '- Remove dupes',
        //   '- Consolidate each category by Priority order',
...
class MagicCards {
  ...
  refresh(resetCooldown = true) {
    ...
    if (resetCooldown) {
      ...
      //   edit.card.cooldown = edit.card?.defaultCooldown ?? cache.settings.cooldown;
  ...
  context() {
    ...
    if (cache.generating) {
      ...
      //   const { info } = MagicCards;
      //   const num = (info.maxChars ?? 2000) - INT.length;
      //   while (TEXT.length <= num) {
      //     const h = History.shift();
      //     if (!h) break;
      //     TEXT += h.text;
      //   }
      //   if (!isBlank(TEXT)) this.text = this.text.replace(rsReg, `Recent_Story:\n${TEXT}`);
      // }
      this.text += INT;
    }
